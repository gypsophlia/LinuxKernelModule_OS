...
/* TODO: add code for handling proc-file */
#define PROC_ENTRY_FILENAME "minix"
struct proc_dir_entry *proc_entry;
extern int  proc_chown(char arg[]);
ssize_t kernelWrite (struct file *file, const char __user *buffer, size_t count, loff_t *offset) 
{
    char buff[501];
    if(copy_from_user(buff,buffer,500)){
        printk (KERN_INFO "Copy from user failed\n" );
        return -EFAULT;
    }
    //pass the string to proc_chown() to process
    if(proc_chown(buff) == -EFAULT){
        printk(KERN_INFO "proc_chown() failed\n");
        return -EFAULT;
    }
    return count;
}
